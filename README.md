...
| Testes dos endpoints | 63 testes, cobertura de ~99% (mínimo imposto: 95%) |
...
|---|---|---|---|
| POST | `/api/v1/books` | Cadastra livro (`Location` no header) | 201, 409, 422 |
...
Erros de validação (422) incluem `errors: [{location, message, type}]`.
...
faz `commit` ou `rollback`. O repositório apenas `flush`a e traduz `IntegrityError`
em `DuplicateBookError` (409). A duplicidade é garantida por constraint no banco, não
por "checar antes de inserir", o que a torna segura contra condições de corrida.
...
`Summary`), `strip_whitespace`, limites de tamanho, `extra="forbid"` e data de
publicação não futura. `BookUpdate` rejeita corpo vazio e `null` explícito. Os
...
para o log e o cliente recebe apenas o `request_id`.
...
- **Dockerfile** multi-stage: o builder instala o Poetry e as dependências (camada
  ...
  não-root (uid 10001), `HEALTHCHECK`, volume `/data` e entrypoint que executa
...
Variáveis `LIBRARY_*` (ver `.env.example`): `ENVIRONMENT`, `DATABASE_URL`,
`LOG_LEVEL`, `LOG_JSON`, `DOCS_ENABLED`.
